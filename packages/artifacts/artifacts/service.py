...
Converts various file types to markdown for easy viewing.
"""

import base64
import re
from typing import Tuple, Optional, Callable, Awaitable, Any
...
AICompleteFunc = Callable[
    [str, str, str, Optional[list]],  # use_case, system_prompt, user_content, images
    Awaitable[str]
]
...
class ArtifactConversionService:
    ...
    def __init__(self, ai_complete: Optional[AICompleteFunc] = None):
        """
        Initialize conversion service.

        Args:
            ai_complete: Optional async function for AI completions.
                         Signature: (use_case, system_prompt, user_content, images) -> str
                         If not provided, image conversion will return a placeholder.
        """
    ...
    async def _convert_image(
        ...
        if not self.ai_complete:
            return (
                f"# {filename}\n\n*Image analysis not available. "
                "AI completion function not configured.*",
                False,
            )
...
Format your response as clean markdown with appropriate headings."""
        ...
        try:
            text = await self.ai_complete(
                "file_to_markdown",
                "You are an expert at analyzing images and converting them to detailed markdown descriptions.",
                prompt,
                [{"media_type": mime_type, "data": base64_content}],
            )
    ...
    def _table_to_markdown(self, table: Any) -> Optional[str]:
