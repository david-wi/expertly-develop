...
export function VoiceTranscription({ tokenUrl, position = 'bottom-right', offset = { x: 24, y: 24 }, autoInject = true, shortcut, onTranscriptionComplete, onInterimTranscript, onError, onStatusChange, className, disabled }: VoiceTranscriptionProps) {
  ...
  const { isConnected, isConnecting, connect, disconnect, sendAudio, error: wsError } = useDeepgramWebSocket({ tokenUrl, onTranscript: handleTranscript, onError: (e) => { updateStatus('error'); onError?.(e) } })
