...
const categories: Category[] = [
  ...
  },
  {
    ...
    features: [
      ...
      { id: 'invoice-pdf', name: 'Invoice PDF export', description: 'Generate professional PDF invoices with your company branding', mcleod: true, trimble: true, mercurygate: true, descartes: true, dat: true, expertly: true, competitorAdvantages: 'McLeod offers 20+ invoice templates. MercuryGate supports multiple invoice formats per customer. Trimble includes load documents as invoice attachments. Standard feature, we\'re on par.' },
      { id: 'email-invoices', name: 'Email invoices directly', description: 'Send invoices via email without downloading and attaching manually', mcleod: true, trimble: true, mercurygate: true, descartes: true, dat: true, expertly: true, competitorAdvantages: 'MercuryGate tracks email opens and when invoices are viewed. Trimble supports scheduled invoice sending. McLeod maintains email history per customer. Consider adding open/view tracking.' },
  ...
  },
  {
    ...
    icon: FolderOpen,
    features: [
      { id: 'doc-upload', name: 'Document upload/storage', description: 'Store all shipment-related documents (BOLs, PODs, invoices, rate confirmations) in one place', mcleod: true, trimble: true, mercurygate: true, descartes: true, dat: true, expertly: true, competitorAdvantages: 'McLeod has unlimited document storage with version history. MercuryGate organizes docs by shipment and document type automatically. Trimble links documents to GL transactions for audit. Our storage is functional but lacks organization features.' },
      { id: 'bol-gen', name: 'BOL generation', description: 'Create Bills of Lading - the legal document describing freight that travels with every shipment', mcleod: true, trimble: true, mercurygate: true, descartes: true, dat: true, expertly: 'planned', competitorAdvantages: 'McLeod has 50+ BOL templates including specialized formats (Hazmat, refrigerated). MercuryGate generates BOLs with shipper-specific requirements automatically. Trimble includes barcode/QR codes for scanning. Standard requirement for operations.' },
      ...
      { id: 'pod-storage', name: 'POD storage', description: 'Store signed proof of delivery documents - required for invoicing and resolving delivery disputes', mcleod: true, trimble: true, mercurygate: true, descartes: true, dat: true, expertly: true, competitorAdvantages: 'MercuryGate extracts delivery time and signature from POD images automatically. Trimble validates POD against expected delivery. McLeod triggers invoicing workflow on POD receipt. We store PODs but lack extraction capabilities.' },
      { id: 'doc-imaging', name: 'Document imaging/scanning', description: 'Scan paper documents or capture photos to create digital records', mcleod: true, trimble: true, mercurygate: true, descartes: true, dat: 'partial', expertly: 'partial', competitorAdvantages: 'Trimble mobile app scans docs with automatic edge detection and enhancement. MercuryGate uses OCR to extract data from scanned docs. McLeod integrates with document scanning services. Should add mobile scanning capability.' },
