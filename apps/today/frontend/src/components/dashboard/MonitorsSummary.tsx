...
const COMMAND_URL = 'https://command.ai.devintensive.com';
...
export function MonitorsSummary() {
  const [monitors, setMonitors] = useState<Monitor[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(false);

  useEffect(() => {
    fetch(`${COMMAND_URL}/api/v1/monitors`, { credentials: 'include' })
      .then((res) => {
  ...
  if (loading) {
    return (
      <Card title="Monitors" action={action}>
    ...
    return (
      <Card title="Monitors" action={action}>
        <p className="text-sm text-gray-500">
          {error ? 'Unable to load monitors.' : 'No monitors configured.'}
        </p>
      </Card>
    );
  }
      ...
      {errorCount > 0 && (
        <div className="mb-3 p-2 bg-red-50 rounded-md">
          <p className="text-xs text-red-700">
            <span className="font-medium">{errorCount}</span> monitor
            {errorCount !== 1 ? 's' : ''} in error state
          </p>
        </div>
      )}
